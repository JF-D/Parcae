...
class ProjectPactumAgent(SimpleElasticAgent): # local scheduler agent
    ...
    def _rendezvous(self, worker_group: WorkerGroup) -> None:
        ...
        while True:
            ...
            # log.error(f'Node: {socket.gethostname()} rendezvous with {group_world_size} workers, rank: {group_rank}, num_pipelines: {num_pipelines}, num_stages: {num_stages}, coordinates: {coordinates}, deci: {global_decision}')
            ...
            # stop keep alive
            if spec.rdzv_handler._rdzv_impl._lease_this_rank_stop is not None:
                spec.rdzv_handler._rdzv_impl._lease_this_rank_stop.set()
    ...
    def _invoke_run(self, role: str = DEFAULT_ROLE) -> RunResult:
        ...
        # overwrite the signal handler registered by the ProcessContext
        # known issue: agent cannot exit until all agents have exited
            ...
            if state == WorkerState.SUCCEEDED:
                ...
                for rank, failure in failures.items():
                    if failure.exitcode == 125:
                        ...
                        break
                if exit_early:
                    log.info(f'[{role}] Worker group {state.name}. spotdl early exit...')
                    continue
                ...
                if self._remaining_restarts > 0:
                    ...
                    log.error(f'[{role}] Worker group {state.name}. stop worker.')
    ...
    def _drain_preempting_workers(self, signum, frame):
        log.error(f'{socket.gethostname()} received SIGTERM {signum}. Draining workers... {self._pcontext}')
    ...
    def _start_workers(self, worker_group) -> Dict[int, Any]:
        ...
        for worker in worker_group.workers:
            ...
            worker_env = {
                ...
                "SPOTDL_NUM_PIPELINES": str(worker.num_pipelines),
                "SPOTDL_NUM_STAGES": str(worker.num_stages),
                "SPOTDL_COORDINATES": json.dumps(worker.coordinates),
            }
    ...
    def _shutdown(self, death_sig: signal.Signals = signal.SIGTERM) -> None:
        log.error(f'Node: {socket.gethostname()} shutdown all processes with {death_sig.name}')
        if self._pcontext:
