...
def run(args):
    ...
    if 'SLURM_JOBID' in os.environ:
        ...
        master_node = f'{node_parts[1]}.{node_parts[2]}.{node_parts[3]}.{node_parts[4]}'

        cur_node_parts = re.findall('[0-9]+', os.environ['SLURMD_NODENAME'])
        cur_node = f'{cur_node_parts[1]}.{cur_node_parts[2]}.{cur_node_parts[3]}.{cur_node_parts[4]}'
        if master_node == cur_node:
            args.role = 'global_scheduler'
        logger.info(f'node: {os.environ["SLURMD_NODENAME"]}, role: {args.role}')
