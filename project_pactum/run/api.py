...
def launch_agent(
    ...
    if config.nproc_per_node > 1:
        logger.info(f'Using multiple processes per node.')
    ...
    if os.environ.get('ONDEMAND_NODE_IP', '') == socket.gethostbyname(socket.gethostname()):
        global_scheduler = True
        logger.info(f'On-demand global scheduler: {socket.gethostname()}')
    rdzv_handler = create_rdzv_handler(rdzv_parameters, global_scheduler=global_scheduler)
    master_addr, master_port = _get_addr_and_port(rdzv_parameters)
    if global_scheduler:
        rdzv_handler.write('/rdzv/ondemand_ip', os.environ['ONDEMAND_NODE_IP'])
    ...
    try:
        if config.project_pactum:
            ...
            extra_env = {
                'SPOTDL_ENABLED': 'ON',
                'SPOTDL_ENDPOINT': config.rdzv_endpoint,
                'SPOTDL_RUN_ID': config.run_id,
                'SPOTDL_MIN_NODES': str(config.min_nodes),
                'SPOTDL_MAX_NODES': str(config.max_nodes),
                'SPOTDL_RDZV_CONFIGS': json.dumps(config.rdzv_configs),
                'SPOTDL_MAX_PIPE_PARALLEL_SIZE': str(config.max_pipe_parallel_size),
                'SPOTDL_DEFAULT_PIPELINE_SIZE': str(config.default_num_stages),
            }
        ...
        if result.is_failed():
            return_values = {}
            for rank, failure in result.failures.items():
                if failure.exitcode == 13:
                    logger.warning(f'Rank {rank} got preempted and exited.')
                    return_values[rank] = None
