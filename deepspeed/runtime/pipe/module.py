...
class PipelineModule(nn.Module):
    def __init__(self,
        ...
        self._setup_comm_grid(num_stages=num_stages, topology=topology)
        ...
        self._partition_layers(method=partition_method, custom_partitions=custom_partitions)

        self.has_redundant_layers = False

        self.tied_modules = nn.ModuleDict()
        self.tied_weight_attrs = {}

        self.param_name_buckets: Dict[int, List[str]] = defaultdict(list)
        self.func_buckets: Dict[int, List[Callable]] = defaultdict(list)
    ...
    def _setup_comm_grid(self, num_stages=None, topology=None, candidate_pool=None):
        # Setup world info
        ...
        self.stage_id = self._grid.get_coord(self.global_rank).pipe

    def update_comm_grid(self, topology, candidate_pool=None):
        self._setup_comm_grid(topology=topology, candidate_pool=candidate_pool)

        # update tied_comms
        self.tied_comms = self._index_tied_modules()

    def _build(self, stage_id, redundant_params=False):
        ...
        for local_idx, layer in enumerate(specs[local_start:local_stop]):
            ...
            # LayerSpec objects contain an nn.Module that should be allocated now.
                ...
                if redundant_params:
                    layer.float()
                funcs.append(layer)
                param_names += [f'{name}.{n}' for n, p in layer.named_parameters() if p.requires_grad]
                if not redundant_params:
                    self.add_module(name, layer)

            # TiedLayerSpec objects contain an nn.Module that should be allocated now.
            elif isinstance(layer, TiedLayerSpec):
                # Build module if we haven't seen it before.
                if layer.key not in self.tied_modules:
                    module = layer.build()
                    # register the module if it isn't redundant
                    if not redundant_params:
                        self.tied_modules[layer.key] = module
                        self.tied_weight_attrs[layer.key] = layer.tied_weight_attr
                else:
                    module = self.tied_modules[layer.key]

                if redundant_params or layer.forward_fn is None:
                    # Just use forward()
                    funcs.append(module)
                else:
                    # User specified fn with args (module, input)
                    funcs.append(partial(layer.forward_fn, module))
            ...
            # LayerSpec objects contain an nn.Module that should be allocated now.
                ...
                if redundant_params:
                    module.float()
                funcs.append(module)
                param_names += [f'{name}.{n}' for n, p in module.named_parameters() if p.requires_grad]
                if not redundant_params:
        ...
        if redundant_params:
            for p in self.redundant_parameters(stage_id):
    ...
    def build_redundant_layers(self):
        # build redundant parameters in CPU
        self.has_redundant_layers = True
        for stage_id in range(self.num_stages - 1):
            self._build(stage_id, redundant_params=True)
    ...
    def redundant_parameters(self, stage_id):
        specs = self._layer_specs
        stage_start_idx = self.parts[stage_id]
        funcs = self.func_buckets.get(stage_id, [])

        for i, module in enumerate(funcs):
            layer_spec = specs[stage_start_idx + i]
            if isinstance(layer_spec, TiedLayerSpec):
                # FIXME: hack GPT model TiedLayerSpec
                continue
                if self.has_redundant_layers and module.position_embeddings.weight.is_cuda:
                    module.position_embeddings.to('cpu')
                yield module.position_embeddings.weight
            else:
    ...
    def _partition_layers(self, method='uniform', custom_partitions=None):
        ...
        if method == 'custom':
            assert custom_partitions is not None, "custom_partitions must be provided"
            self.parts = custom_partitions
        elif method == 'uniform':
            ...
            if method.endswith('transformer'):
                self.parts = ds_utils.partition_transformer_manually(weights=binary_weights,
                                                                     num_parts=num_stages)
            elif method.endswith('bottleneck'):
                self.parts = ds_utils.partition_resnet_manually(weights=binary_weights,
                                                                num_parts=num_stages)
            elif method.endswith('bertlayer'):
                self.parts = ds_utils.partition_bert_manually(weights=binary_weights,
                                                              num_parts=num_stages)
            else:
    ...
    def get_new_partition(self, new_num_stages):
        ...
        if method == 'uniform':
            ...
            for idx in self._find_layer_type(layertype):
                binary_weights[idx] = 1
            if method.endswith('transformer'):
                parts = ds_utils.partition_transformer_manually(weights=binary_weights,
                                                                num_parts=new_num_stages)
            elif method.endswith('bottleneck'):
                parts = ds_utils.partition_resnet_manually(weights=binary_weights,
                                                           num_parts=new_num_stages)
            elif method.endswith('bertlayer'):
                parts = ds_utils.partition_bert_manually(weights=binary_weights,
                                                              num_parts=new_num_stages)
            else:
    ...
    def load_layers(self, received_state, prev_model_state={}):
        ...
        for i, l_id in enumerate(range(self.parts[self.stage_id], self.parts[self.stage_id + 1])):
            layer_spec = self._layer_specs[l_id]
            if isinstance(layer_spec, TiedLayerSpec):
            ...
            if hasattr(module, 'load_state_dict'):
                if l_id in prev_model_state:
                    ...
                    module.load_state_dict(lyr_state_dict)
                    del lyr_state_dict
                    continue
    ...
    def load_redundant_layers(self, prev_state, prev_redundant_state):
        for stage_id in range(self.num_stages - 1):
            bucket = self.func_buckets[stage_id]
            for i, l_id in enumerate(range(self.parts[stage_id], self.parts[stage_id + 1])):
                layer_spec = self._layer_specs[l_id]
                if isinstance(layer_spec, TiedLayerSpec):
                    module = self.tied_modules[layer_spec.key]
                    continue
                ...
                if hasattr(module, 'load_state_dict'):
                    if l_id in prev_state:
                        lyr_state_dict = prev_state[l_id][0]
                        module.load_state_dict(lyr_state_dict)
                        del lyr_state_dict
                        continue

                    if l_id in prev_redundant_state:
                        lyr_state_dict = prev_redundant_state[l_id][0]
                        module.load_state_dict(lyr_state_dict)
                        del lyr_state_dict
                        continue

                    print(f'[WARNING] Layer {l_id} not found in either prev_state or prev_redundant_state')
    ...
    def _is_checkpointable(self, funcs):
        ...
        if self.class_name in ('GPTModelPipe', 'GPT2ModelPipe'):
