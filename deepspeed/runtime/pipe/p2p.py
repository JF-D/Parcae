...
def send(input, dest_stage, async_op=False):
    global _groups
    ...
    if async_op:
        global _async
        op = dist.isend(input, dest_rank)
        _async.append(op)
    else:

        if can_send_recv():
            return dist.send(input, dest_rank)
        else:
            group = _get_send_recv_group(src_stage, dest_stage)
            src_rank = _grid.stage_to_global(stage_id=src_stage)
            return dist.broadcast(input, src_rank, group=group, async_op=async_op)


def recv(input, src_stage, async_op=False):
    global _groups
    assert async_op == False, "Doesn't support async_op true"
    ...
    if async_op:
        global _async
        op = dist.irecv(input, src_rank)
        _async.append(op)
    else:
        if can_send_recv():
            ret = dist.recv(input, src_rank)
        else:
            group = _get_send_recv_group(src_stage, dest_stage)
            ret = dist.broadcast(input, src_rank, group=group, async_op=async_op)
...
def isend(tensor, dest_stage, group=None):
    src_stage = _grid.get_stage_id()
    # _is_valid_send_recv(src_stage, dest_stage)

    dest_rank = _grid.stage_to_global(stage_id=dest_stage)
    return dist.isend(tensor, dest_rank, group=group)


def irecv(tensor, src_stage, group=None):
    dest_stage = _grid.get_stage_id()
    # _is_valid_send_recv(src_stage, dest_stage)

    src_rank = _grid.stage_to_global(stage_id=src_stage)
    return dist.irecv(tensor, src_rank, group=group)
