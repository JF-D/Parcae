...
def partition_transformer_manually(weights, num_parts):
    num_items = len(weights)
    # First check for the trivial edge case
    if num_items <= num_parts:
        return partition_uniform(num_items, num_parts)

    num_transformer_layers = sum(weights)
    # the final logits and loss computation is roughly equal to 1 trnasformer layer
    num_layers_per_partition = (num_transformer_layers + 1) // num_parts
    parts_dist = [num_layers_per_partition] * num_parts
    for i in range((num_transformer_layers + 1) % num_parts):
        parts_dist[-i-2] += 1
    ...
    for i, w in enumerate(weights):
        part_weights += w
        if part_weights >= parts_dist[part_idx]:
            parts[part_idx + 1] = i + 1
            part_idx += 1
            part_weights = 0
    parts[-1] = num_items
    return parts
...
def partition_resnet_manually(weights, num_parts):
    num_items = len(weights)
    # First check for the trivial edge case
    if num_items <= num_parts:
        return partition_uniform(num_items, num_parts)

    total_layers = sum(weights) + 2
    block_size = total_layers // num_parts
    parts_dist = [block_size] * num_parts
    for i in range(total_layers % num_parts):
        parts_dist[-2-i] += 1
...
def partition_bert_manually(weights, num_parts):
    num_items = len(weights)
    # First check for the trivial edge case
    if num_items <= num_parts:
        return partition_uniform(num_items, num_parts)

    num_bert_layers = sum(weights)
    # the final logits and loss computation is roughly equal to 1 trnasformer layer
    num_layers_per_partition = num_bert_layers // num_parts
    parts_dist = [num_layers_per_partition] * num_parts
    for i in range(num_bert_layers % num_parts):
        parts_dist[-i-2] += 1
    parts_dist[-1] += 1
