...
class DeepSpeedEngine(Module):
    ...
    def __init__(
        ...
        self.skip_optimizer_check = skip_optimizer_check
        ...
        self.join = True #'SPOTDL_JOIN' in os.environ
        if dist.spotdl_enabled:
            self.group_rank = int(os.environ['GROUP_RANK'])
            self.max_pipe_parallel_size = os.environ['SPOTDL_MAX_PIPE_PARALLEL_SIZE']
            ...
            if dist.get_rank() == 0:
                for rank in range(dist.get_world_size()):
                    ...
                    self.global_store.set(f'exception-{rank}', '0')
        ...
        if not self.skip_optimizer_check:
    ...
    def _del_client_model(self):
        # remove client model from _modules
        modules = self.__dict__.get('_modules')
        del modules['module']
        # remove module attribute from engine
    ...
    def clip_fp32_gradients(self):
        total_norm = clip_grad_norm_(parameters=self.module.parameters(),
                                     max_norm=self.gradient_clipping(),
                                     mpu=self.mpu)
        return total_norm

    def _take_clip_gradients(self):
        if self.gradient_clipping() > 0.0:
            total_norm = None
            if not (self.fp16_enabled() or self.bfloat16_enabled() or self.amp_enabled()
                    or self.zero_optimization()):
                total_norm = self.clip_fp32_gradients()
            elif self.amp_enabled():
                # AMP's recommended way of doing clipping
                # https://nvidia.github.io/apex/advanced.html#gradient-clipping
                master_params = amp.master_params(self.optimizer)
                total_norm = clip_grad_norm_(parameters=master_params,
                                             max_norm=self.gradient_clipping(),
                                             mpu=self.mpu)
            return total_norm

    def _take_model_step(self, lr_kwargs, block_eigenvalue={}, clip_gradients=True):
        if clip_gradients:
            self._take_clip_gradients()
