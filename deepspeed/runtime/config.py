...
class DeepSpeedConfig(object):
    ...
    def _batch_assertion(self):
        ...
        try:
            ...
            if os.environ.get('SPOTDL_ENABLED', None) != 'ON':
                raise

            import math

            # hack to support free batch size
            grad_acc = train_batch // micro_batch
            grad_acc = math.ceil(grad_acc / self.world_size)

            self.gradient_accumulation_steps = grad_acc
            self.train_batch_size = micro_batch * grad_acc * self.world_size
            print(f'[WARNING] SPOTDL enabled, adjusting batch size from {train_batch} to {self.train_batch_size}, '
