...
class TorchBackend(Backend):
    """
        ...
        needed.
    """
    def __init__(self, backend, timeout, init_method, rank=-1, world_size=-1, store=None, name='torch'):
        ...
        self.init_process_group(backend, timeout, init_method, rank=rank, world_size=world_size, store=store)

    def init_process_group(self, backend, timeout, init_method, rank=-1, world_size=-1, store=None):
    ...
    def new_group(self, ranks, timeout=None, backend=None):
        if timeout is None:
            timeout = self.timeout
        return torch.distributed.new_group(ranks, timeout=timeout, backend=backend)
